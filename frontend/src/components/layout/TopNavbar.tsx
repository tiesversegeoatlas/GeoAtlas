...
export function TopNavbar() {
  ...
  const alertEvent = events.find((event) => ["critical", "high"].includes(event.riskLevel)) || events[0];
        ...
        <div className="hidden lg:flex items-center gap-6 mr-6 border-r border-border pr-6">
          ...
          <div className="flex flex-col items-end">
            ...
            <Badge variant="outline" className={error
              ? "bg-critical/10 text-critical border-critical/20 gap-1 h-5 text-[10px] font-bold uppercase py-0 px-2"
              : "bg-low/10 text-low border-low/20 gap-1 h-5 text-[10px] font-bold uppercase py-0 px-2"
            }>
              <div className={`w-1.5 h-1.5 rounded-full ${error ? "bg-critical" : "bg-low"}`} />
              {error ? "API unavailable" : loading ? "Syncing" : "Operational"}
            </Badge>
          </div>
        </div>

        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-critical/10 text-critical text-[10px] font-bold uppercase tracking-wider border border-critical/20">
          <Activity className="w-3 h-3" />
          <span>{alertEvent ? `Latest: ${alertEvent.title}` : "Awaiting live collection data"}</span>
        </div>
